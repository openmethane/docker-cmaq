...
C $Header: /Volumes/Data/CVS/CMAQ_CVSrepos/CCTM/src/aero/aero4/hetchem.f,v 1.1.1.1 2010/06/14 16:02:58 sjr Exp $
...
c              parameterization of Davis et al (2007), by default.
c
c  REFERENCES:
...
c      ammonium, sulfate, and nitrate.  Atmos. Chem. Phys. Disc., in
c      press, 2007.
...
C *** ARGUMENTS
      ...
      REAL, INTENT( INOUT ) :: CBLK( NSPCSDA ) ! main array of variables
      REAL, INTENT( OUT ) :: GAMMA      ! N2O5->NO3 reaction probability
      REAL, INTENT( IN ) :: AIRTEMP     ! Air temperature [ K ]
      REAL, INTENT( IN ) :: AIRRH       ! Fractional relative humidity
...
C *** molecular weights, hardcoded to match values in mechanism files
      ...
      REAL, PARAMETER :: MWHNO3 = 63.0    ! molecular weight for HNO3
      REAL, PARAMETER :: MWN2O5 = 108.0   ! molecular weight for N2O5

C *** conversion factors for thermodynamics vars

      REAL, PARAMETER :: FAERHNO3 = 1.0E-6 / MWHNO3  ! for ug  -> mole
      REAL, PARAMETER :: FAERN2O5 = 1.0E-6 / MWN2O5  ! for ug  -> mole
      REAL, PARAMETER :: DFHNO3 = 1.0 / FAERHNO3     ! for mole  -> ug
...
C *** UPDATE CBLK ARRAY
c      HNO3 and N2O5 concentrations are changed in this subroutine.
...
c    state, based on the parameterization by Davis et al. (2007).
c
c  KEY SUBROUTINES CALLED: none
c
...
C *** molecular weights, hardcoded to match values in mechanism files
      ...
      REAL, PARAMETER :: MWNO3 = 62.0     ! molecular weight for NO3
      REAL, PARAMETER :: MWSO4 = 96.0     ! molecular weight for SO4
      REAL, PARAMETER :: MWNH4 = 18.0     ! molecular weight for NH4
...
C *** variables for default parameterization of N2O5PROB 
      ...
      REAL      T293,T287 ! temperature threshold variables

C *** statement function for inverting the logit transformation given
C     in Eq 7 by Davis et al (2007)
...
c     et al (2007), in which N2O5PROB is a function of RH, TEMP, 
...
c     Davis et al (2007).
         ...
         IF ( CRYSTAL ) THEN
            ...
            LAMD     = -6.133764 + 3.5920 * RH        ! Eq 9
     &                - 0.196879 * T293
            GAMD     = MIN( LOGITINV(LAMD), 0.0124 )  ! Eq 10
            N2O5PROB = ( X1 + X2 ) * GAMD             ! Eq 13
     &                + X3 * MIN( GAMD, GAM3 )

c *** Compute N2O5PROB on aqeuous particles using Eqs 4, 5, 8, and 12
c     by Davis et al (2007).

         ELSE
            T287     = MAX( 0.0, TEMP - 287.0 )
            LAM1     = -4.559088 + 2.8593 * RH        ! Eq 4
     &                - 0.111201 * T287
            LAM2     = LAM1 - 0.369769                ! Eqs 4 & 5
...
c   PVB 11/05/07 Coded the first version.
      ...
      USE AERO_INFO_AE4      ! Module containing CBLK indices
...
C     slightly from STDTEMP and STDATMPA in the AERO_INFO_AE4 module.
