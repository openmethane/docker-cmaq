...
C $Header: /Volumes/Data/CVS/CMAQ_CVSrepos/CCTM/src/aero/aero5_txhg/aero_subs.f,v 1.1.1.1 2010/06/14 16:03:00 sjr Exp $
...
C *** ARGUMENTS
      ...
      REAL PHGRATE           ! Gas-phase mercuric aerosol production rate  [ ug / m**3 s ]
...
C:WTH variables for mercruy aerosols

      REAL, PARAMETER :: DIFFPHG  = 1.12E-05  ! diffusivity of elemental Hg, m^2/s
      REAL, PARAMETER :: ALPHPHG  = 1.0       ! Accommodation coefficient for Hg aero
      REAL, PARAMETER :: MWPHG    = 200.59E-3 ! molecular weight of Hg 
      REAL, SAVE      :: COFCBAR_PHG          ! molecular velocity coefficient for Hg
      REAL, SAVE      :: CBAR_PHG             ! molecular velocity for Hg
      REAL            :: DV_PHG               ! molecular diffusivity of Hg vapors
                                              ! after temp and pressure correction 

      REAL*8  AM0AT, AM0AC ! zeroeth moments
      REAL*8  AM1AT, AM1AC ! first moments
      REAL*8  AM2AT, AM2AC ! second moments

      REAL*8 FCONCAT_PHG( 2 )  ! Aitken mode 2nd and 3rd moments
      REAL*8 FCONCAC_PHG( 2 )  ! Accumulation mode 2nd and 3rd moments
      REAL*8 FCONCCO_PHG( 2 )  ! Coarse mode 2nd and 3rd moments
      REAL*8 FCONCM1_PHG       ! reciprocals of total mercury cond rates

      REAL*8 OMEGA_AT_PHG    ! Aitken mode 2nd and 3rd moments
      REAL*8 OMEGA_AC_PHG    ! Accumulation mode 2nd and 3rd moments
      REAL*8 OMEGA_PHG( 2 )  ! partitioning coefficient for equilibrium PM mass

      REAL*8 CONDPHG(NMODES)      ! mercury condensation rate [ ug/m**3 s ]
      REAL*8 RATE_PHG             ! COND mercury cond rate
...
C *** calculate minimum values for number and 2nd moment.
     ...
     &              EXP(2.0 * LOG(SGINICO)**2)
         ...
         COFCBAR_PHG = SQRT( 8.0 * RGASUNIV/( PI * MWPHG ))
...
C:WTH applying involatile and no coarse mode processes of Hg
...
c     violations arising from numerical error.
      ...
      DV_PHG   = DIFFPHG  * DIFFCORR
C Calculate Hg molecular velocity (temperature dependant)
      CBAR_PHG = COFCBAR_PHG * SQRT_TEMP  
...
C *** Set second moments
      ...
      CALL HCOND3(AM0AT,AM1AT,AM2AT,DV_PHG,ALPHPHG, CBAR_PHG,
     &            FCONCAT_PHG )                
      CALL HCOND3(AM0AC,AM1AC,AM2AC,DV_PHG,ALPHPHG, CBAR_PHG,
     &            FCONCAC_PHG )   

      FCONCM1_PHG  = 1.0 / ( FCONCAT_PHG(2) + FCONCAC_PHG(2) )
      OMEGA_AT_PHG = FCONCM1_PHG * FCONCAT_PHG(2)
      OMEGA_AC_PHG = 1.0 - OMEGA_AT_PHG
...
c     due to intermodal coagulation is treated here.
      ...
      IF( MERCURY_MECH )THEN

C:WTH: assumes all precursor goes to J mode

         TMASS = CBLK(VPHGI) + CBLK(VPHGJ) + PHGRATE * DT
         PROD = 0.0  ! previous version, POL equaled PHGRATE*OMEGA_AT_PHG
         POL = PROD * LOSSINV
         CBLK( VPHGI ) = POL + ( CBLK( VPHGI ) - POL ) * EXPDT
         CBLK( VPHGI ) = MAX(CONMIN, CBLK( VPHGI ) )
         CBLK( VPHGJ ) = MAX( CONMIN, TMASS - CBLK( VPHGI ) )
...
c     renaming.
      ...
      PHM3 = 0.0   ! initialize mode merging coefficients
      FM3 = 0.0
...
C:WTH
      CALL AEROPROC_TRACE( EXPDT, PHM3, FM3)   ! update trace aerosol species
C:WTH
               ...
               IF ( CFINAL( ISP,IMODE ) .LT. 0.0D0 ) THEN
                  ...
                  IF ( CFINAL( ISP, 1 ) .LT. 0.0D0 ) THEN
                     IF ( ABS( CFINAL( ISP,1 ) ) .LT. 1.0D-15 ) THEN
                        CFINAL( ISP,1 ) = 0.0D0
                     ELSE
                        WRITE(LOGDEV,*) 'Too much evaporation: aero_subs.f'
                        CFINAL( ISP,1 ) = 0.0D0
!                       STOP
                     ENDIF
                  ENDIF 
...
c *** double precision vars for ISORROPIA
         ...
         CALL CALC_H2O( WI, RHI, TEMPI, H2O_NEW, LOGDEV ) 
...
c-----------------------------------------------------------------------
      ...
      SUBROUTINE CALC_H2O (WI, RH, T, H2O_NEW, LOGDEV)
...
c     for ZSR calculation below
      ...
      ELSE
        WRITE(LOGDEV,*) 'aero_subs.f: case not supported ',
     ...
     &        Cinf(KNH4) - Cinf(KNO3) - Cinf(KCL) - 2.d0 * CH2SO4
         ...
         elseif (-cc < small .AND. 0.d0 <= bb) then  
           ...
           if (bb**2-4.d0*aa*cc < 0) then
              WRITE(LOGDEV,*) 'Compute_Flux, sqrt<0'
              Qk = 0.d0
           endif
         endif
