...
C $Header: /Volumes/Data/CVS/CMAQ_CVSrepos/CCTM/src/aero/aero5_sulf/aero_subs.f,v 1.1.1.1 2010/06/14 16:03:00 sjr Exp $
...
c 
c SJR 01/31/06 Added capability for sulfate tracking
c
...
c      Conference, Chapel Hill, NC, 2004.
     ...
     &                     SO4RATE, SO4RATEI, ORGPROD, NPSPCS, 
     &                     XLM, AMU,
     &                     DGATK, DGACC, DGCOR,
...
C...track the gas phase production
      ...
      TMASS = CBLK( VSO4AIGAS ) + CBLK( VSO4AJGAS )
      CBLK( VSO4AIGAS ) = CBLK( VSO4AIGAS ) * EXPDT
      CBLK( VSO4AJGAS ) = MAX( CONMIN, TMASS - CBLK( VSO4AIGAS ) )

C...track the emissions contribution

      TMASS = CBLK( VSO4AIEMIS ) + CBLK( VSO4AJEMIS )
      CBLK( VSO4AIEMIS ) = CBLK( VSO4AIEMIS ) * EXPDT
      CBLK( VSO4AJEMIS ) = MAX( CONMIN, TMASS - CBLK( VSO4AIEMIS ) )

C...track the ic/bc contribution

      TMASS = CBLK( VSO4AIICBC ) + CBLK( VSO4AJICBC )
      CBLK( VSO4AIICBC ) = CBLK( VSO4AIICBC ) * EXPDT
      CBLK( VSO4AJICBC ) = MAX( CONMIN, TMASS - CBLK( VSO4AIICBC ) )
...
c     using Equation 11b of Binkowski and Roselle (2003).
         ...
         CBLK( VSO4AJEMIS) = CBLK( VSO4AJEMIS) + CBLK( VSO4AIEMIS) * FM3
...
c     using Equation 11e of Binkowski and Roselle (2003).
         ...
         CBLK( VSO4AIEMIS) = CBLK( VSO4AIEMIS) * PHM3
...
C     on Equations A21 and A23 of Binkowski & Shankar (1995).
     ...
     &                       + FCONCCO_SO4( 2 ) )
         ...
         CBLK( VSULF ) = XH2SO4
ccccc    CBLK( VSULFP) = XH2SO4 ! adjust CBLK value
         CBLK( VSULFP) = 0.0 ! adjust CBLK value
...
C *** Add water and SOA to 2nd moment while preserving standard deviation
     ...
     &                              - CFINAL( KCL,IMODE) ) * HCLRAT 
            ...
            CBLK( VSO4K ) = SO4
            CBLK( VSO4AKGAS ) = CBLK( VSO4AKGAS ) + RATE * TSTEP * H2SO4RATM1
            CBLK( VH2OK ) = H2O
            CBLK( VHPLUSK ) = HPLUS
            CBLK( VCO2 )  = WETM2
...
C *** Add water and SOA to 2nd moment while preserving standard deviation
     ...
     &         + M3OTHR
         ...
         IF ( IMODE .EQ. 1 ) THEN
            ...
            CBLK( VSO4AI )  = SO4
            CBLK( VSO4AIGAS ) = CBLK( VSO4AIGAS ) + RATE * CONVFAC
            CBLK( VH2OAI )  = H2O
     ...
     &                      -       CBLK( VNH4AI ) / MWNH4
         ELSE IF ( IMODE .EQ. 2 ) THEN
            ...
            CBLK( VSO4AJ )  = SO4
            CBLK( VSO4AJGAS ) = CBLK( VSO4AJGAS ) + RATE * CONVFAC
            CBLK( VH2OAJ )  = H2O
            CBLK( VAC2 )    = WETM2   
            CBLK( VHPLUSJ ) = 2.0 * CBLK( VSO4AJ ) / MWSO4
     &                      +       CBLK( VNO3AJ ) / MWNO3
